...
- 文献 BibTeX 自动化
  - 支持通过 DOI 从 Crossref 获取 BibTeX 条目
  - 可选择输出到指定 `.bib` 文件
- 附带实用工具
  - 文件重命名、简单文件格式转换、文件搜索
  - CSV / Excel 数据绘图与统计分析
...
```bash
python cli.py bib --doi 10.1038/s41586-021-03819-2
```

直接保存到 `references.bib`：

```bash
python cli.py bib --doi 10.1038/s41586-021-03819-2 --save
```

保存到指定文件：

```bash
python cli.py bib --doi 10.1038/s41586-021-03819-2 --save --output refs/references.bib
```
...
- DOI 转 BibTeX 依赖 Crossref 网络服务，离线或服务不可用时无法获取结果。
